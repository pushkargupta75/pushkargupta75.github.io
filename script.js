document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- Animated Counters (Modified to skip CGPA, Projects, and Certifications) ---
    const animateCounters = () => {
        statNumbers.forEach(counter => {
            const parent = counter.closest('.stat-item');
            const isCounterElement = parent && (
                parent.textContent.toLowerCase().includes('cgpa') ||
                parent.textContent.toLowerCase().includes('projects') ||
                parent.textContent.toLowerCase().includes('certifications') ||
                parent.textContent.toLowerCase().includes('major projects')
            );
            
            // Skip animation for CGPA, Projects, and Certifications
            if (isCounterElement) {
            ...
            // Animate other counters normally
    ...
    // --- Contact Form with EmailJS (Modified with page reload) ---
    document.getElementById('contact-form').addEventListener('submit', function(e) {
        ...
        // First, try sending just the main notification email
        emailjs.send('service_vge7po1', 'template_8x4ija9', notificationParams)
        .then(function(response) {
            console.log('Email sent successfully:', response);
            
            // Clear the form immediately after successful send
            form.reset();
            
            // Show success notification
            showNotification('Message sent successfully! Thank you for reaching out.', 'success');
            
            // Optional: Send auto-reply (don't let this fail the main process)
            ...
            .catch(function(autoReplyError) {
                console.log('Auto-reply failed, but main message was sent:', autoReplyError);
            });
        ...
        .catch(function(error) {
            ...
            if (error.status === 400) {
                ...
                errorMessage += 'Please try again or email me directly at pushkargupta993@gmail.com';
            }
    ...
    function showNotification(message, type = 'info') {
        ...
        // Hide notification after 5 seconds (only for non-success messages since success will reload)
        if (type !== 'success') {
            setTimeout(() => {
